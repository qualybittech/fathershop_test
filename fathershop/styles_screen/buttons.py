...
from selenium_helpers import login, check_cache_btn, no_element_dropdown, create_new_btn, duplicate, delete, \
    filter_search, edit, color, gradient, camera, image_options, save, click_back_btn, add_breakpoint_btn
...
class Buttons:
    ...
    def navigate_to_button(self):
        try:
            ...
            typo = wait_for_element(self.wait, (By.XPATH, "//a[normalize-space()='Button']"))
...
class TestFathershopTheme:
    ...
    def test_styles_functions(self, setup):
        ...
        button.login()
        button.navigate_to_styles_screen()
        button.navigate_to_button()
        button.check_cache_btn()
        button.duplicate()
        button.delete()
        button.no_element_dropdown()
        button.filter_search("Te")
        button.create_new_btn()
        button.new_label(" Test")
        button.open_general()
        button.camera()
        button.gradient()
